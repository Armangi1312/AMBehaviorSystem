	...
	public class BaseGraphView : GraphView, IDisposable
	{
        ...
        void ViewTransformChangedCallback(GraphView view)
        {
            if (graph != null)
            {
                ...
                graph.position = new Vector3(
                    t.x.unit == LengthUnit.Pixel ? t.x.value : 0f,
                    t.y.unit == LengthUnit.Pixel ? t.y.value : 0f,
                    0f
                );
                graph.scale = new Vector3(sc.value.x, sc.value.y, 1f);
            }
        }
