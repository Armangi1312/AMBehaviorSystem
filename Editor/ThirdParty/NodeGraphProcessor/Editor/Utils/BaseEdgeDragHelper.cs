...
namespace GraphProcessor
{
    public class BaseEdgeDragHelper : EdgeDragHelper
    {
        ...
        public override void Reset(bool didConnect = false)
        {
            ...
            if (wasPanned)
            {
                if (!resetPositionOnPan || didConnect)
                {
                    ...
                    Vector3 p = container.resolvedStyle.translate;
                    Vector3 s = new Vector3(
                        container.resolvedStyle.scale.value.x,
                        container.resolvedStyle.scale.value.y,
                        1f
                    );
        ...
        private void Pan(TimerState ts)
        {
            ...
            Translate t = container.resolvedStyle.translate;
            Scale sc = container.resolvedStyle.scale;

            Vector3 currentPos = new Vector3(t.x.value, t.y.value, 0f);
            Vector3 s = new Vector3(sc.value.x, sc.value.y, 1f);

            graphView.UpdateViewTransform(currentPos - panDiff, s);
