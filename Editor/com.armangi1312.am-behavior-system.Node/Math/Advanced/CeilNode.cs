...
namespace AMBehaviorSystem.Node.Math.Advanced
{
    ...
    public class CeilNode : BaseNode, IMathNode, ISourceGenerationNode
    {
        ...
        public void Generate(SourceContext context)
        {
            ...
            string name = $"ceil_{GUIDParse.GetGUIDParse(GUID)}";
            Type outType = input.Type == typeof(double) ? typeof(double) : typeof(float);

            Argument argument = new(input.Type, input.Name);

            string template = input.Type == typeof(double) ? "Math.Ceiling(#)" : "MathF.Ceiling(#)";
